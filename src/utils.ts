...
export const isNotDefined = <T>(val: T): boolean =>
...
export const isStringTuple = (tuple: StringTuple): boolean =>
  tuple.length === 2 && isString(tuple[0]) && isString(tuple[1]);

export const isValidTarget = (anchorTarget: Target): boolean =>
  targetList.findIndex((t) => t === anchorTarget) === -1;

export const isValidMethod = (method: Method): boolean =>
  methodList.findIndex((m) => m === method) === -1;

export const removeInvalidValues = (data: StringTuple[]): StringTuple[] =>
  data.filter((d) => isStringTuple(d));
...
export const getWidthOrHeight = (arg: string | number): number => {
  ...
  throw new Error(`${arg} is not a supported width/height type.`);
};
...
export const getValidAttributes = (data?: DataAttributes): StringTuple[] => {
  if (isNotDefined(data) || (Array.isArray(data) && data.length === 0))
    return [];
  if (!Array.isArray(data && data[0]) && isStringTuple(data as StringTuple))
    return [data as StringTuple];
...
export const removeFromArray = <T>(array: T[], target: T): T[] =>
  array.filter((a) => a !== target);

export const mergeAttrArray = (
  attrArray: StringTuple[],
  newAttr: StringTuple
): StringTuple[] => {
...
export const updateElement = (
  ...
  if (element) {
    ...
    dataAttributes.forEach(([key, data]) =>
